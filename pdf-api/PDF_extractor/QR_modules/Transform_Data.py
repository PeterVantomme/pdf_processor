...
## Main method (called by helper.py)
def transform_file(file):
    ...
    finally: del image, pdf

## Necessary to transform Pixmap generated by fitz to numpy array needed for opencv transformations
def pix2np(pix):
...
def transform_png(image):
    ...
    #Transformations
    #Using same variable prevents memory leak since it gets overwritten.
    #Look at opencv documentation for explanation about tranformations.
    #There's a notebook called "QR-cleaner.ipynb" which contains code that can be used for experimentation.
